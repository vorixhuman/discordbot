    ...
    async def add(self, ctx, role: discord.Role):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ?', (ctx.guild.id,)) as cursor:
                ...
                if row:
                    embed = discord.Embed(
                                          description=f"<:MekoExclamation:1449445917500510229> VC role is already set in this guild with the role {ctx.guild.get_role(row[0]).mention}.\nPlease **remove** it to add another one.", color=0x000000)
                    await ctx.reply(embed=embed)
                    return
            await db.execute('INSERT INTO vcroles (guild_id, role_id) VALUES (?, ?)', (ctx.guild.id, role.id))
            await db.commit()
            embed = discord.Embed(
                                  description=f"<a:H_TICK:1449446011490537603> Successfully added VC role {role.mention} for this guild.", color=0x000000)
            await ctx.reply(embed=embed)
    ...
    async def remove(self, ctx, role: discord.Role):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ? AND role_id = ?', (ctx.guild.id, role.id)) as cursor:
                ...
                if not row:
                    embed = discord.Embed(
                                          description="<:MekoExclamation:1449445917500510229> Given role is not set in VC role.", color=0x000000)
                    await ctx.send(embed=embed)
                    return
            await db.execute('DELETE FROM vcroles WHERE guild_id = ? AND role_id = ?', (ctx.guild.id, role.id))
            await db.commit()
            embed = discord.Embed(
                                  description=f"<a:H_TICK:1449446011490537603> Successfully removed VC role {role.mention} for this guild.", color=0x000000)
            await ctx.send(embed=embed)
    ...
    async def config(self, ctx):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ?', (ctx.guild.id,)) as cursor:
                ...
                if not row:
                    embed = discord.Embed(
                                          description="<:MekoExclamation:1449445917500510229> VC role is not set in this guild.", color=0x000000)
                    await ctx.send(embed=embed)
                    return
