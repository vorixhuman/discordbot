...
async def initialize_greet_db():
    global greet_db
    if greet_db is None:
        greet_db = await aiosqlite.connect('database/welcome.db')
        await greet_db.execute("PRAGMA journal_mode=WAL")  # Enable WAL mode for better concurrency
        await greet_db.commit()

async def close_greet_db():
    global greet_db
    if greet_db:
        await greet_db.close()
        greet_db = None
...
class greet(commands.Cog):
    ...
    async def process_queue(self, guild):
        while self.join_queue[guild.id]:
            ...
            await initialize_greet_db()
            async with greet_db.execute("SELECT welcome_type, welcome_message, channel_id, embed_data, auto_delete_duration FROM welcome WHERE guild_id = ?", (guild.id,)) as cursor:
